		...
		try (Session session = driver.session()) {
			return session.executeRead(tx -> {
				...
				return result.stream()
						.collect(Collectors.toMap(record -> AppUtils.nodeToCustomer(record.get("c").asNode()),
...
//	MATCH (c:Customer)-[r:PURCHASED]->(o:Order) RETURN c, count(o) as count LIMIT 25
	...
	public Map<Customer, Long> listCustomersWithTheNumberOfOrders2() {
		...
		try (Session session = driver.session()) {
			return session.executeRead(tx -> {
				...
				return result.stream()
						.collect(Collectors.toMap(record -> AppUtils.nodeToCustomer(record.get("c").asNode()),
								record -> record.get("count").asLong(), (v1, v2) -> v1, () -> new LinkedHashMap<>()));
			});
		}
	}
	...
	public Map<Customer, Long> listCustomersWithTheNumberOfOrders4() {
		...
		try (Session session = driver.session()) {
			return session.executeRead(tx -> {
				...
				return result.stream()
						.collect(Collectors.toMap(record -> AppUtils.nodeToCustomer(record.get("c").asNode()),
								record -> record.get("count").asLong(), (v1, v2) -> v1,
	...
	/**
	 * Number of customers by city, sorted by number of customers descending Thống
	 * kê số khách hàng theo từng thành phố (sắp xếp theo số khách hàng giảm dần).
	 * 
	...
	public Map<String, Long> numberOfCustomersByCity() {
		...
		try (Session session = driver.session()) {
			return session.executeRead(tx -> {
				Result result = tx.run(query);
				return result.stream().map(record -> record.asMap()).collect(Collectors.toMap(
						p -> (String) p.get("city"), p -> (Long) p.get("count"), (a, b) -> a, LinkedHashMap::new));
			});
		}
	}

	/**
	 * Number of customers by city, sorted by city name ascending Thống kê số khách
	 * hàng theo từng thành phố (sắp xếp theo city).
	 * 
	 * @return Map<String, Long>
	 */
	public Map<String, Long> numberOfCustomersByCity2() {
		...
		try (Session session = driver.session()) {
			return session.executeRead(tx -> {
				Result result = tx.run(query);
				return result.stream().map(record -> record.asMap()).collect(Collectors
						.toMap(p -> (String) p.get("city"), p -> (Long) p.get("count"), (a, b) -> a, TreeMap::new));
			});
		}
	}
	...
	public static void main(String[] args) {
		...
		dao.listCustomersWithTheNumberOfOrders4().entrySet().stream().forEach(entry -> {
