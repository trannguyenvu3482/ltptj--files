...
//	https://community.neo4j.com/t/timestamps-between-java-and-neo/63160/15
	/**
	 * Find order by orderID Tìm đơn hàng theo orderID
	 * 
	 * @param orderID
	 * @return Order
	 */
	...
	/**
	 * Get the total amount of an order by orderID Tính tổng số tiền của một đơn
	 * hàng khi biết orderID MATCH (o:Order)-[r:ORDERS]->(p:Product) where
	 * o.orderID="11005" RETURN sum(toFloat(r.unitPrice)*r.quantity) as totalAmount
	 * 
	 * @param orderID
	 * @return
	 */
	...
	/**
	 * Get the total amount of an order by order year Thống kê tổng tiền hóa đơn
	 * theo tháng / năm Match (o:Order)-[r:ORDERS]->(p:Product) where
	 * o.orderDate.year=1996 and o.orderDate.month=7 RETURN
	 * sum(toFloat(r.unitPrice)*r.quantity) as totalAmount
	 * 
	 * @param year
	 * @return
	...
	public double totalAmountOfAnOrderByYear(int year, int month) {
		String query = "Match (o:Order)-[r:ORDERS]->(p:Product) " + "where o.orderDate.year=$year "
				+ "and o.orderDate.month=$month " + "RETURN sum(toFloat(r.unitPrice)*r.quantity) as totalAmount";
